...
pair<double, double> getXY(double s, double d, const vector<double> &maps_s,
                           const vector<double> &maps_x,
                           const vector<double> &maps_y) {
  int prev_wp = -1;
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          /* Where I'm now */
          pair<double, double> ref(car_x, car_y);
          /* Where I'm heading */
          ...
          if (prev_size < 2) {
             ...
             * that */
            ref = make_pair(previous_path_x[prev_size - 1],
                            previous_path_y[prev_size - 1]);

            pair<double, double> ref_prev(previous_path_x[prev_size - 2],
          ...
          for (auto &pt : pts) {
            ...
            pt = make_pair(
                ...
                shift.first * sin(-ref_yaw) + shift.second * cos(-ref_yaw));
          }

          /* Let's go to smooth the path */
          tk::spline s;
          s.set_points(pts);
